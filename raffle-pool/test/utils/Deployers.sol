...
abstract contract Deployers {
    ...
    IUniswapV4Router04 baseRouter; // Store the base router for token approvals
    ...
    function deployToken() internal returns (MockERC20 token) {
        ...
        // Also approve the base router in case the SenderRelayRouter forwards calls
        if (address(baseRouter) != address(0)) {
            token.approve(address(baseRouter), type(uint256).max);
        }

        permit2.approve(address(token), address(positionManager), type(uint160).max, type(uint48).max);
        permit2.approve(address(token), address(poolManager), type(uint160).max, type(uint48).max);
        // Approve Permit2 for the base router to pull tokens via SenderRelayRouter
        if (address(baseRouter) != address(0)) {
            permit2.approve(address(token), address(baseRouter), type(uint160).max, type(uint48).max);
        }
    }
    ...
    function deployRouter() internal virtual {
        if (block.chainid == 31337) {
            baseRouter = IUniswapV4Router04(payable(V4RouterDeployer.deploy(address(poolManager), address(permit2))));
        } else {
            baseRouter = IUniswapV4Router04(payable(AddressConstants.getV4SwapRouterAddress(block.chainid)));
        }
        
        // Wrap the base router with SenderRelayRouter to pass through msg.sender
    ...
    function deployArtifacts() internal {
        ...
        deployRouter(); // Must be called before deployToken() so baseRouter is set
    }
}
